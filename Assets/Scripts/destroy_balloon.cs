...
namespace Valve.VR.InteractionSystem
{
	...
	public class destroy_balloon : MonoBehaviour
	{
        ...
        {
            if (this.gameObject.name == "Balloon_EX(Clone)" || this.gameObject.name == "Balloon_third(Clone)" )
            {
                //Debug.Log(this.gameObject.name);
                //audioData = GetComponent<AudioSource>(); // play the bomp voice
                //audioData.Play(0);
                //GameManager.instance.Getscore(1, 1);
                FindObjectOfType<GameManager>().Getscore(1, 1);
            }
            else if (this.gameObject.name == "Balloon_EX_black(Clone)" || this.gameObject.name ==  "Banana(Clone)" )
            {
                //Debug.Log(other.name);
                //audioData = GetComponent<AudioSource>();
                //audioData.Play(0);
                //GameManager.instance.Getscore(1, 2);
                FindObjectOfType<GameManager>().Getscore(1, 2);
            }
